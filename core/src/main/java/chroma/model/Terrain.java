...
public class Terrain extends ObstacleSprite {
    ...
    public Terrain(TextureRegion region, float[] points, float units) {
        super();
        this.region = region;
        this.units = units;
...
//        this.polygon.setScale(units, units);
    ...
    public void draw(SpriteBatch batch) {
        if (region == null) { super.draw(batch); return; }

        Rectangle bounds = polygon.getBoundingRectangle();
        float px = bounds.x * units;
        float py = bounds.y * units;
        float pw = bounds.width  * units;
        float ph = bounds.height * units;

        batch.draw(region, px, py, pw, ph);
    }
